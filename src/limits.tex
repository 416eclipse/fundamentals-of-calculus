...
\begin{center}
	\begin{tabular}{| >{\color{mathGreen}}c| >{\color{mathGreen}}c| >{\color{mathGreen}}c| >{\color{mathGreen}}c| >{\color{mathGreen}}c|| >{\color{mathGreen}}c| >{\color{mathGreen}}c| >{\color{mathGreen}}c| >{\color{mathGreen}}c|}
		\hline
...
\begin{marginfigure}
	\centering
	\resizebox{\linewidth}{!}{%
		\input{rawfigs/ch-limits/limits-function-example-graph.tex}%
	}
...
While this definition may seem grounded enough (it is, for the sake of an introduction), we can more rigorously define the limit using the \textit{Epsilon-Delta ($\epsilon-\delta$) definition of the limit} in the next section. For now, let us evaluate limits.
...
\begin{thm}\label{thm:lims}
	...
	\end{enumerate}
\end{thm}
Fortunately, these theorems are intuitive. It is not unreasonable to assume that a function going to one limit, when added to another function with another limit, results in the sum of their respective limits.
...
\begin{marginfigure}
	\centering
	\resizebox{\linewidth}{!}{%
		\input{rawfigs/ch-limits/limits-sin-graph.tex}%
	}
...
\begin{exmp}
	...
	\begin{soln}
		...
		\begin{align*}
			...
			&= 2\left(\lim_{x\to 0} x\right) + 4 + \frac{\left(\displaystyle\lim_{x\to 0} x\right)\left(\sqrt{4+\left(\displaystyle\lim_{x\to 0} x\right)}\right)}{2} \\
...
\begin{exmp}
	...
	\begin{soln}
		...
		Thus, we have to simplifying by recalling factorization.
		\bmarginnote{Recall the sum of cubes which is $x^3 + a^3 = (x + a)(x^2 - ax + a^2)$.}
		\begin{align*}
...
\end{defn}
Epsilon-Delta proofs involve first finding and choosing a $\delta$ value, and then beginning the formal proof with the chosen $\delta$ value. Let us prove Example \ref{eqn:firstexmp} rigorously.
\begin{exmp}
...
\begin{marginfigure}
	\centering
	\resizebox{\linewidth}{!}{%
		\input{rawfigs/ch-limits/limits-heaviside.tex}%
	}
